...
import { Send, CheckCircle, XCircle, Loader2, Upload, X, Plus, Layers, BarChart3, Copy } from 'lucide-react';
import {
  messageApi,
  contactApi,
...
export function MessageTester() {
  ...
  // ── Tab layout: Compose | Multi-session Bulk | Analytics ──
  const [tab, setTab] = useState<'compose' | 'bulk' | 'analytics'>('compose');

  // Multi-session bulk: one message fanned across several ready sessions round-robin. Each session
  // gets its own async batch, so progress is tracked per (session, batch) pair and polled together.
  const [bulkSessionIds, setBulkSessionIds] = useState<string[]>([]);
  const [bulkUseRegistry, setBulkUseRegistry] = useState(true);
  ...
  const [bulkBatches, setBulkBatches] = useState<
    Array<{ sessionId: string; sessionName: string; batchId: string; status: BatchStatus; total: number; sent: number; failed: number; pending: number }>
  >([]);
  const bulkPollRefs = useRef<Record<string, ReturnType<typeof setInterval>>>({});
  ...
  const { data: statsData } = useStatsMessagesQuery(statsPeriod);
  const { data: registryContacts = [] } = useRegistryContactsQuery(2000);
  const { data: sessionReplies = [] } = useRegistryRepliesQuery();

  // Stop all multi-bulk polling on unmount; the batches keep running server-side regardless.
  useEffect(() => {
    return () => {
      Object.values(bulkPollRefs.current).forEach(clearInterval);
    };
  }, []);

  const stopBulkPoll = (key: string) => {
    const timer = bulkPollRefs.current[key];
    if (timer) {
      clearInterval(timer);
      delete bulkPollRefs.current[key];
    }
  };

  const bulkRecipientIds = bulkUseRegistry
    ? registryContacts.map(c => (c.phone.includes('@') ? c.phone : `${c.phone}@c.us`))
    : parseBulkRecipients(bulkCustomText);

  // Round-robin the recipient list across the selected sessions so no single session hammers a
  // contiguous slice — burst 1 of every session first, then burst 2, and so on.
  const roundRobin = (items: string[], sessionsCount: number): string[][] => {
    const buckets: string[][] = Array.from({ length: sessionsCount }, () => []);
    items.forEach((item, i) => buckets[i % sessionsCount].push(item));
    return buckets;
  };

  // Analytics KPIs derive totals from the per-session breakdown (MessageStats has no top-level totals).
  const analyticsTotals = (statsData?.bySession ?? []).reduce(
    (acc, s) => ({ sent: acc.sent + s.sent, received: acc.received + s.received }),
    { sent: 0, received: 0 },
  );

  const handleBulkDispatch = async () => {
    if (!bulkContent.trim() || bulkSessionIds.length === 0 || bulkRecipientIds.length === 0) return;
    setBulkBatching(true);
    setBulkBatches([]);
    const sessionLookup = new Map(allSessions.map(s => [s.id, s.name]));
    const buckets = roundRobin(bulkRecipientIds, bulkSessionIds.length);

    // Fire every session's batch concurrently (202-fast), then poll all until terminal.
    const created = await Promise.all(
      bulkSessionIds.map(async (sessionId, idx) => {
        const recipientChatIds = buckets[idx] ?? [];
        const sessionName = sessionLookup.get(sessionId) ?? sessionId;
        if (recipientChatIds.length === 0) {
          return { sessionId, sessionName, batchId: '', status: 'completed' as BatchStatus, total: 0, sent: 0, failed: 0, pending: 0 };
        }
        const messages: BulkMessageItem[] = recipientChatIds.map(chatId => ({
          chatId,
          type: 'text',
          content: { text: bulkContent },
        }));
        const batch = await messageApi.sendBulk(sessionId, { messages });
        return { sessionId, sessionName, batchId: batch.batchId, status: 'pending' as BatchStatus, total: batch.totalMessages, sent: 0, failed: 0, pending: batch.totalMessages };
      }),
    );
    setBulkBatches(created);

    // Poll each running batch on its own interval; run totals tick as statuses arrive.
    for (const entry of created) {
      if (!entry.batchId) continue;
      bulkPollRefs.current[entry.batchId] = setInterval(async () => {
        try {
          const status = await messageApi.getBatchStatus(entry.sessionId, entry.batchId);
          setBulkBatches(prev =>
            prev.map(p => (p.batchId === status.batchId ? { ...p, status: status.status, sent: status.progress.sent, failed: status.progress.failed, pending: status.progress.pending } : p)),
          );
          if (TERMINAL_BATCH_STATUSES.includes(status.status)) stopBulkPoll(entry.batchId);
        } catch {
  ...
  return (
    <div className="message-tester">
      ...
      <div className="tester-tabs" role="tablist" aria-label={t('messageTester.title')}>
        <button
          type="button"
          role="tab"
        ...
        >
          <Send size={15} /> {t('messageTester.tabs.compose')}
        </button>
        <button
          type="button"
          role="tab"
          ...
          onClick={() => setTab('bulk')}
        >
          <Layers size={15} /> {t('messageTester.tabs.bulk')}
        </button>
        <button
          type="button"
          role="tab"
        ...
        >
          <BarChart3 size={15} /> {t('messageTester.tabs.analytics')}
        </button>
      </div>

      {tab === 'bulk' ? (
        <div className="bulk-panel">
          <h2 className="eyebrow">{t('messageTester.multiBulk.title')}</h2>
          <p className="hint">{t('messageTester.multiBulk.subtitle')}</p>

          <div className="form-group">
            <span className="group-label" id="mt-bulk-sessions-label">
              {t('messageTester.multiBulk.sessionsLabel')}
            </span>
            <div className="bulk-sessionpool" role="group" aria-labelledby="mt-bulk-sessions-label">
              {sessions.length === 0 ? (
                <small>{t('messageTester.multiBulk.noReady')}</small>
              ) : (
                sessions.map(s => (
                  <label key={s.id} className="bulk-sessionpool__item">
                    <input
                      type="checkbox"
                      checked={bulkSessionIds.includes(s.id)}
                      onChange={() =>
                        setBulkSessionIds(prev => (prev.includes(s.id) ? prev.filter(id => id !== s.id) : [...prev, s.id]))
                      }
                    />
          ...
          <div className="form-group">
            <label className="checkbox-label">
              <input type="checkbox" checked={bulkUseRegistry} onChange={e => setBulkUseRegistry(e.target.checked)} />
              {t('messageTester.multiBulk.useRegistry')}
            </label>
            <span className="hint">{t('messageTester.multiBulk.useRegistryHint')}</span>
          </div>

          {bulkUseRegistry ? (
            <div className="form-group">
              <span className="hint">{t('messageTester.multiBulk.recipientsCount', { count: registryContacts.length })}</span>
            </div>
          ) : (
            <div className="form-group">
              <label htmlFor="mt-bulk-list">{t('messageTester.multiBulk.customList')}</label>
              <textarea
                id="mt-bulk-list"
                value={bulkCustomText}
                onChange={e => setBulkCustomText(e.target.value)}
                placeholder="+62812345678, Alice"
                rows={4}
              />
            </div>
          )}
          ...
          <div className="form-group">
            <label htmlFor="mt-bulk-msg">{t('messageTester.messageContent')}</label>
            <textarea
              id="mt-bulk-msg"
              value={bulkContent}
          ...
          <button
            ...
            disabled={!canWrite || bulkBatching || bulkSessionIds.length === 0 || bulkRecipientIds.length === 0 || !bulkContent.trim()}
          >
            {bulkBatching ? <Loader2 className="animate-spin" size={18} /> : <Copy size={18} />}
            {bulkBatching
              ? t('messageTester.multiBulk.dispatching')
              : t('messageTester.multiBulk.dispatch', { sessions: bulkSessionIds.length })}
          </button>

          {bulkBatches.length > 0 && (
            <div className="bulk-progress">
              <h3>{t('messageTester.multiBulk.perSessionProgress')}</h3>
              <div className="bulk-progress__table">
                {bulkBatches.map(b => {
                  const pct = b.total > 0 ? Math.round(((b.sent + b.failed) / b.total) * 100) : 100;
                  return (
                    <div key={b.batchId} className="bulk-progress__row">
                      <span className="bulk-progress__name">{b.sessionName}</span>
                      <span className={`batch-badge ${b.status}`}>{t(`messageTester.batch.status.${b.status}`)}</span>
                      <div className="batch-progress-bar">
                        <div className="batch-progress-fill" style={{ width: `${pct}%` }} />
                      </div>
                      <span className="bulk-progress__nums">
                        {t('messageTester.multiBulk.sent')} {b.sent} · {t('messageTester.multiBulk.failed')} {b.failed} ·{' '}
                        {t('messageTester.multiBulk.pending')} {b.pending}
                      </span>
                    </div>
                  );
                })}
              </div>
              {bulkBatches.length > 0 && bulkBatches.every(b => TERMINAL_BATCH_STATUSES.includes(b.status)) && (
                <p className="hint">{t('messageTester.multiBulk.allDone')}</p>
              )}
            </div>
          )}
        </div>
        ...
        <div className="analytics-panel">
          <h2 className="eyebrow">{t('messageTester.analytics.title')}</h2>

          <div className="form-group">
            <label htmlFor="mt-analytics-period">{t('messageTester.analytics.periodLabel')}</label>
            <select
              id="mt-analytics-period"
              value={statsPeriod}
          ...
          <div className="analytics-kpis">
            <div className="analytics-kpi">
              <span>{t('messageTester.analytics.sentTotal')}</span>
              <strong>{analyticsTotals.sent}</strong>
            </div>
            <div className="analytics-kpi">
              <span>{t('messageTester.analytics.receivedTotal')}</span>
              <strong>{analyticsTotals.received}</strong>
            </div>
            <div className="analytics-kpi">
              <span>{t('messageTester.analytics.replyRate')}</span>
              <strong>
                {analyticsTotals.sent > 0 ? `${Math.round((analyticsTotals.received / analyticsTotals.sent) * 100)}%` : '—'}
              </strong>
            </div>
          </div>

          {(statsData?.bySession?.length ?? 0) > 0 ? (
            <div className="analytics-table">
              <h3>{t('messageTester.analytics.bySession')}</h3>
              <table>
                <thead>
                  <tr>
                    <th>{t('messageTester.analytics.session')}</th>
                    <th>{t('messageTester.analytics.sent')}</th>
                    <th>{t('messageTester.analytics.received')}</th>
                    <th>{t('messageTester.analytics.replied')}</th>
                    <th>{t('messageTester.analytics.blockedReported')}</th>
                  </tr>
                </thead>
                <tbody>
                  {statsData?.bySession.map(s => {
                    const rr = sessionReplies.find(r => r.sessionId === s.sessionId);
                    return (
                      <tr key={s.sessionId}>
                        <td>{s.name}</td>
                        <td>{s.sent}</td>
                        <td>{s.received}</td>
                        <td>{rr ? rr.replied : 0}</td>
                        <td>{rr ? rr.blocked + rr.reported : 0}</td>
                      </tr>
                    );
                  })}
                </tbody>
            ...
            <p className="hint">{t('messageTester.analytics.noData')}</p>
          )}
        </div>
      ) : (
            ...
            <>
              <div className="form-group">
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.5rem' }}>
                  <label htmlFor="mt-11" style={{ marginBottom: 0 }}>{t('messageTester.bulkRecipients')}</label>
                  <button type="button" className="browse-btn" onClick={() => bulkCsvInputRef.current?.click()} style={{ padding: '0.25rem 0.5rem', fontSize: '0.75rem' }}>
